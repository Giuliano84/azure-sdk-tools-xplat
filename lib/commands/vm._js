...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .execute(function(dnsName, image, userName, password, options, callback) {
      ...
      var vmSize;
      if('admin' === userName)
          callback(new Error($('Invalid <user-name>, it cannot be equal to "admin", choose a different one')));
  ...
  function createVM(options, cmdCallback) {
    ...
    if (options.role) {
      ...
      if (options.community) {
        progress = cli.progress($('Looking up community image'));
        var managementEndPoint = cli.category('account').managementEndpointUrl();
    ...
    function createDefaultRole(name, callback) {
      ...
      function createDefaultRoleEnsurePassword_() {
        ...
        if (typeof options.password === 'undefined' && options.noSshPassword !== true) {
          interaction.password(cli, 'Enter VM \'' + options.userName + '\' password: ', '*', function(pass) {
            process.stdin.pause();
  ...
  function listVMs(options, cmdCallback) {
    var vmClient = new VMClient(cli, cli.category('account').lookupSubscriptionId(options.subscription));
    vmClient.getDeployments(options, function(error, deployments) {
      ...
      interaction.formatOutput(self.cli, vms, function(outputData) {
  ...
  function deleteVM(name, options, _) {
    ...
    if (found) {
      if (!options.quiet && !interaction.confirm(cli, util.format($('Delete the VM %s ? [y/n] '), name), _)) {
        return;
      }
  ...
  function endpointCreateDelete(options, cmdCallback) {
    ...
    var vmClient = new VMClient(cli, cli.category('account').lookupSubscriptionId(options.subscription));
    vmClient.getDeployments(options, function(error, deployments) {
      if (errors) { return cmdCallback(error); }
      else {
        var found = null;
            ...
            if (!error) {
              ...
              if (m !== endpointCount) {
                ...
                if (options.create) {
                  ...
                  progress.end();
                  cmdCallback(new Error($('Endpoint not found in the network configuration')));
                }
              }
  ...
  function endpointCreateMultiple(options, cmdCallback) {
    ...
    vmClient.getDeployments(options, function(error, deployments) {
      if (error) { return cmdCallback(error); }
        ...
        // got unique role under a deployment and service, update the network configuration
        if (found) {
          var progress = cli.progress($('Reading network configuration'));
          utils.doServiceManagementOperation(channel, 'getRole', found.svc, found.deploy.Name, options.name, function(error, response) {
            progress.end();
